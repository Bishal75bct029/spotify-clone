            ...
            {playlistDetail.length > 0 && playlistDetail[1].image && (
              <>
                ...
                <div className=" capitalize mt-auto mb-auto ">
                  <h1 className="text-white text-[30px] tracking-wider mb-[14px] font-bold">
                    {playlistDetail[1].description}
                  </h1>
          ...
          <div className="mt-8 ml-4 cursor-pointer">
            {playlistDetail[0].map((song: any) => {
              return (
                <div
                  key={song.id}
                  className="flex mb-3 items-center hover:bg-[#1e1e1e] duration-200 active:bg-[#2e2e2e]"
                  onClick={() => playMusic(song.playUrl)}
                >
                  <img src={`${song.image}`} alt="Not Found" className="h-20 w-20 rounded-sm" />
                  <div className="m-4">
                    <p className="text-[#a7a7a7]  text-[14px]">{song.name ? song.name.replace(/&quot;/g, '"') : ''}</p>
                  ...
                  <div className="text-[#a7a7a7] text-[12px] ml-auto">{getDuration(song.durations)}</div>
                  <div className="text-[#a7a7a7] mx-10 ">
                    <RiDownloadLine />
                  </div>
                </div>
              );
